...
- ```/etc/pki/CA``` should be owned by ```root:root``` and have ```0700```
  permissions:  
```bash
...
- Create an empty certificate index and a serial file to indicate next
  certificate number to be issued:  
```bash
...
- Install ```httpd``` and ```mod_ssl``` and make the certificates available over
  the network  
```bash
...
### Observations  
```my-ca.crt``` in ```/var/www/html/certs``` must have SELinux context
```httpd_sys_content_t```.  
